...
def test_playlist_view():
    ...
    client = APIClient()
    client.force_authenticate(user=CustomUser(username="test", password="test"))
    response = client.get('/quiz/playlists/')
    assert response.status_code == 200
    assert response.json() == {
        'test': [{'id': playlist1.id, 'title': 'test', 'image_link': ''}, {'id': playlist2.id, 'title': 'test2', 'image_link': ''}],
        'other': [{'id': playlist3.id, 'title': 'test3', 'image_link': ''}]
    }
